...
std::string/*wstring*/ CasaLens::get_date()
{
    time_t t = time(0);
    struct tm now;
...
pplx::task<json::value> CasaLens::get_movies(const utility::string_t& postal_code)
{
    ...
    }).then([=](json::value movie_result)
    {
        try
        {
            ...
            auto date = get_date();
            std::string/*wstring*/ year = date.substr(0, date.find(U("-"))); 
...
void CasaLens::fetch_data(http_request message, const std::string/*wstring*/& postal_code, const std::string/*wstring*/& location)
{
...
void CasaLens::get_data(http_request message, const std::string/*wstring*/& input_text)
{
    if (!is_number(utility::conversions::to_utf8string(input_text)))
    {
        std::string/*wstring*/ bing_maps_url(casalens_creds::bmaps_url);
